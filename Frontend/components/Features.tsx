...
const features = [
  {
    title: "Data Structures & Algorithms",
    description:
      "Master DSA fundamentals with structured lessons, curated problems, and detailed explanations to build a strong base.",
    tag: "DSA",
    image: Code,
...
export const Features = () => {
  return (
    <section
      id="features"
      className="bg-background text-foreground py-24 px-4 sm:px-8 relative overflow-hidden"
    >
      <div className="max-w-3xl mx-auto text-center mb-16 relative z-10">
        <h2 className="text-4xl sm:text-5xl font-extrabold bg-gradient-to-r from-purple-800 to-purple-600 text-transparent bg-clip-text">
          Empower Your Coding Journey
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto relative z-10">
        {features.map((feature) => (
          <div
            key={feature.title}
            className="relative p-6 rounded-xl border border-[var(--card-border)] bg-[var(--card-background)] shadow-md transition-all duration-300 hover:scale-[1.03] cursor-pointer hover:shadow-[0_5px_20px_var(--card-shadow)]"
          >
            <div
              className={`inline-block px-3 py-1 text-xs font-medium rounded-full bg-gradient-to-r ${feature.color} text-white mb-4`}
            >
            ...
            <div className="w-full h-36 relative mb-4 flex items-center justify-center rounded-lg bg-[var(--card-softBackground)]">
              <Image
                src={feature.image}
                alt={feature.title}
                width={100}
